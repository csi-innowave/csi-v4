...
const GalleryImage = ({ imageUrl, index, setLightboxIndex }: { imageUrl: string, index: number, setLightboxIndex: (index: number) => void }) => {
  ...
  return (
    <div 
      className="mb-6 w-full cursor-none group relative" 
...
function Gallery() {
  ...
  const optimizeCloudinaryUrl = (url: string): string => {
    ...
    if (url.includes('/upload/')) {
      ...
      // This will drastically reduce image size (e.g. 5MB -> 50KB) and fix network lag.
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (spotlightRef.current) {
        // Spotlight strictly follows mouse
    ...
    const updateCursor = () => {
      // Lerping for smooth trailing custom cursor
          ...
          <div className="w-full md:w-11/12 xl:w-10/12">
            ...
            <div className="pb-20">
              ...
              {loading ? (
                ...
                <>
                  {/* Stable DOM Masonry mimicking Pinterest using actual column flexboxes, preventing CSS rebalancing pop-in */}
                  <div 
                    className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 cursor-none relative z-10"
